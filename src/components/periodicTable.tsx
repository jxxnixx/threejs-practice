...
function createSpheresFromData({ data, bigRadius }: any) {
	...
	data.forEach((element: any) => {
		...
		const position = new THREE.Vector3().setFromSphericalCoords(bigRadius, phi, theta)
...
// blur 처리 방식 고민하기 - 대상 피사체 바로 뒤에 blur layer 까는 방식 느낌...? 2개 정도..
// 카메라를 이동시킬건지 블러 레이어를 이동시킬건지 고민

// 상태관리 객체화/클래스화 or 함수화
// jotai처럼 배열 안에 obj 만들고 그 안에서 상태관리 해도 됨. jotai는 three 객체도 가질 수 있을지도? 확인 ㄱㄱ
	...
	return (
		...
		>
			...
			<Suspense fallback={null}>
				<Bounds observe>
					<SelectToZoom>
						<BigSphere period={period1} smaillRadius={20} bigRadius={100} color={'red'} />
						<BigSphere period={period2} smaillRadius={20} bigRadius={200} color={'red'} />
						<BigSphere period={period3} smaillRadius={20} bigRadius={300} color={'orange'} />
						<BigSphere period={period4} smaillRadius={20} bigRadius={400} color={'yellow'} />
						<BigSphere period={period5} smaillRadius={20} bigRadius={500} color={'green'} />
						<BigSphere period={period6} smaillRadius={20} bigRadius={600} color={'blue'} />
						<BigSphere period={period7} smaillRadius={20} bigRadius={700} color={'purple'} />
					</SelectToZoom>
				</Bounds>
				{/* <OrbitControls
          autoRotate={false}
